...
function _getText($: ReturnType<typeof cheerio.load>, el: any | any[]): string {
  if (Array.isArray(el)) {
    ...
    logger.warn(
      `Received an unknown element while extracting content from HTML files. This should never happen. Please open an issue at https://github.com/anmode/docusaurus-search-local/issues if you see this message (debug: got type ${el.type}).`
    );
    return "";
  }
}
